...
describe("pino-aws-cloudwatch", function () {
  ...
  it("should not send logs to CloudWatch Logs", function (done) {
    var inStream = fs.createReadStream(
      path.resolve(__dirname, "../mocks/log_empty.txt")
    );
    var pump = require("pump");
    var split = require("split2");
  ...
  it("should emit a flushed event", function (done) {
    var inStream = fs.createReadStream(
      path.resolve(__dirname, "../mocks/log_single.txt")
    );
    var pump = require("pump");
    var split = require("split2");
